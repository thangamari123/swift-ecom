  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (error.code === 'auth/email-already-in-use') {
        ...
        setAuthError('Your password should be at least 6 characters long.');
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans">
      <div className="w-full max-w-md bg-white rounded-[2rem] p-6 sm:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
        ...
        <div className="flex border-b border-slate-100 mb-8">
          <Link to="/login" className="flex-1 pb-3 text-center font-medium text-slate-500 hover:text-slate-800">
            Login
          </Link>
          <Link to="/register" className="flex-1 pb-3 text-center font-bold text-[#6157d6] border-b-2 border-[#6157d6]">
            Register
          </Link>
        </div>
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[13px] text-slate-500 font-medium">Join us and start shopping</p>
        </div>
        ...
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
           <div className="relative">
             ...
             />
             {errors.fullName && <p className="text-red-500 text-xs mt-1 pl-1 font-medium">{errors.fullName.message as string}</p>}
           </div>
           ...
           <div className="flex items-center pt-1">
             <label className="flex items-center gap-2 cursor-pointer group">
               <div className="relative flex items-center justify-center">
                 <input 
                   type="checkbox" 
                   className="peer appearance-none w-4 h-4 border border-slate-300 rounded focus:outline-none focus:ring-2 focus:ring-[#6157d6]/20 checked:bg-[#6157d6] checked:border-[#6157d6] transition-colors" 
                   {...register('terms')}
                 />
               ...
               <span className="text-[13px] font-medium text-slate-600 group-hover:text-slate-800 transition-colors">I agree to Terms & Conditions</span>
             </label>
           </div>
        ...
        <div className="mt-8 text-center text-[13px] font-medium text-slate-600">
          Already have an account? <Link to="/login" className="font-bold text-[#6157d6] hover:text-[#4d45a9] transition-colors">Login</Link>
        </div>
      </div>
    </div>
  );
